...
static tS16 Xvalue;
static tS16 Yvalue;
static tS16 refXvalue;
static tS16 refYvalue;
struct snakeSegment
{
  tS32 row;
  tS32 col;
  ...
  do
  {
    ...
    do
    {
      ...
      osSleep(speed * PAUSE_LENGTH);
	      Xvalue = getAnalogueInput1(ACCEL_X);
      ...
      high_score = score;
          {
            // tU8 stop = stopTimer();
            //printf("\n Timer: %d\n", stop);
            tU8 bestScoreBuffer[10];
            tS8 isOk = eepromPageRead(EEPROM_ADDR, &bestScoreBuffer, 7);
            osSleep(20);
            //tU8 bestTime = 0xff*bestTimeBuffer[4] + bestTimeBuffer[5];
            //printf("\nEEPROMREAD: %d\n", bestTime);
            tU8 isBest = 0;
            eepromPoll();
            osSleep(20);
            if (high_score > score || high_score == 0) {
                ...
                newBestScore[3] = 'K';
                newBestScore[5] = 0x0;
                eepromWrite(EEPROM_ADDR, &newBestScore, 5);
                eepromPoll();
                osSleep(20);
            }
        }
...
void addSegment()
{
  if (Yvalue - refYvalue >= 15) //left
  {
    snake[snakeLength].row = snake[snakeLength - 1].row;
    snake[snakeLength].col = snake[snakeLength - 1].col - 1;
  }
  if (Xvalue - refXvalue <= -15) //up
  {
    snake[snakeLength].row = snake[snakeLength - 1].row - 1;
    snake[snakeLength].col = snake[snakeLength - 1].col;
  }
  if (Yvalue - refYvalue <= -15) //right
  {
    snake[snakeLength].row = snake[snakeLength - 1].row;
    snake[snakeLength].col = snake[snakeLength - 1].col + 1;
  }
  if (Xvalue - refXvalue >= 15) //down
  {
    snake[snakeLength].row = snake[snakeLength - 1].row + 1;
    snake[snakeLength].col = snake[snakeLength - 1].col;
  }
